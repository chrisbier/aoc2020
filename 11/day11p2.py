...
class SeatMatrix:
    ...
    def calculate_position_matrix(self, limit):
        initial = [(0,0)]
        run_list = ['tl', 'tc', 'tr']
        tl = (-1, -1)
        tc = ( 0, -1)
    ...
    def surrounded(self, column, row):
        ...
        #relative_matrix = position_matrix
        max_range = self.max_row if self.max_row > self.max_column else self.max_column

        for circle_radius in range(0, max_range):
            for (p_column, p_row) in position_matrix:
                p_column_new = p_column + ((p_column + p_column) * circle_radius)
                p_row_new = p_row + ((p_row + p_row) * circle_radius)
                if p_column > -2 and p_row > -2 and p_column <= self.max_column + 1 and p_row <= self.max_row + 1:
                    print(self.get_seat_value(column + p_column, row + p_row))
                    if self.get_seat_value(column + p_column_new, row + p_row_new) == '#':
                        position_matrix_dict.update({(p_column, p_row): True})
                        count = count + 1
                else:
                    break
                if False in position_matrix_dict.values():
                    break
            if False in position_matrix_dict.values():
                break
        return count
